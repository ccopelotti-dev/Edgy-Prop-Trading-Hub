...
if (content.includes(menu_current)) {
    content = content.replace(menu_current, menu_new);
} else {
    console.log('Error: Could not find Menu section to replace.');
}

// 3. Content wrappers
const content_start = `<Content style={{ margin: '24px 24px', overflow: 'initial' }}>
                                    <div style={{ marginBottom: 24 }}>`;
const content_start_new = `<Content style={{ margin: '24px 24px', overflow: 'initial' }}>
                                  <div style={{ display: activeMenu === '1' ? 'block' : 'none' }}>
                                    <div style={{ marginBottom: 24 }}>`;
content = content.replace(content_start, content_start_new);

const content_end = `                                        </Card>
                                    )}
                                </Content>`;
const content_end_new = `                                        </Card>
                                    )}
                                  </div>
...
if (content.includes(content_end)) {
    content = content.replace(content_end, content_end_new);
} else {
    console.log('Error: Could not find Content end to replace.');
}

// 4. Modals append
const modal_current = `                            </Form>
                        </Modal>

                    </ConfigProvider>`;
const modal_new = `                            </Form>
                        </Modal>
...
if (content.includes(modal_current)) {
    content = content.replace(modal_current, modal_new);
} else {
    console.log('Error: Could not find Modal end to replace.');
}

fs.writeFileSync('index.html', content, 'utf8');
console.log('Successfully ran replacements!');
